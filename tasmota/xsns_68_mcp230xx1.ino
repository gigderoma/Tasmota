            ...
            if ((intf >> intp) & 0x01) { // we know which pin caused interrupt
              ...
              if (Settings.MCP230xx1_config[intp+(MCP230xx1_port*8)].pinmode > 1) {
                ...
                if (report_int) {
                  ...
                  unsigned long millis_since_last_int = millis_now - int_millis[intp+(MCP230xx1_port*8)];
                  int_millis[intp+(MCP230xx1_port*8)]=millis_now;
...
bool MCP230xx1_Command(void)
{
  ...
  if (validpin && (paramcount > 1)) {
    if (!strcmp(subStr(sub_string, XdrvMailbox.data, ",", 2), "?")) {
      ...
      char intmodetxt[9];
      sprintf(intmodetxt,IntModeTxt(Settings.MCP230xx1_config[pin].int_report_mode));
